...
- [ ] **Fase H** — Road to Namecheap: arreglos finales + deploy vía FTP ← **siguiente**
  ...
  - [ ] **H8** — Preparación del export estático para FTP: build de producción + carpeta «en producción» lista para subir; elegir app FTP open-source y documentar config. **OJO:** activar `images:{ unoptimized:true }` en `next.config.ts` (lo necesitan `next/image` del Hero y las OG de `next/og`); re-verificar que `robots.txt`/`sitemap.xml`/`llms.txt`/OG salgan en `out/`. Prompt `prompts/8. Preparación del sitio para subir vía ftp.txt` ← **siguiente**
  - [ ] **H9** — Deploy a Namecheap vía FTP + verificación en enmachile.com. Tras confirmar, abrir **Fase I** (indexación rápida) replicando la estructura de prompts de la Fase H. Prompt `prompts/9. Subida del sitio web a hosting de Namecheap.txt`
