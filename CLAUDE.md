...
- [ ] **Fase H** — Road to Namecheap: arreglos finales + deploy vía FTP ← **siguiente**
  ...
  - [ ] **H5** — Cambios página Proyectos (reducir a 1 proyecto «Turbina eólica de baja escala», card 16:9 horizontal a ancho máximo, quitar pills de tipo en card y Hero de detalle) ← **siguiente** · prompt `prompts/5. Cambio en la página Proyectos.txt`
  - [ ] **H6** — Hero de la landing (NUEVA): reemplazar el video del Hero (render con zoom-out de ~8 min en loop, hoy placeholder) por **imagen(es) HD con efecto**, más livianas y fáciles de cambiar, con posibilidad de **varias al azar**. Probar primero con una imagen tipo Unsplash. Prompt a actualizar/mejorar: `prompts/6. Cambio de hero en Landing.txt`
