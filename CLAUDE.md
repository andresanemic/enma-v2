...
- [ ] **Fase H** — Road to Namecheap: arreglos finales + deploy vía FTP ← **siguiente**
  ...
  - [ ] **H7** — Optimización SEO (sitemap.xml, metadata, OG, preparar Google Search Console; plugin `claude-seo`). GEO/AEO. Prepara archivos para la indexación rápida de la futura Fase I. Prompt `prompts/7. Optimización SEO.txt` ← **siguiente**
  - [ ] **H8** — Preparación del export estático para FTP: build de producción + carpeta «en producción» lista para subir; elegir app FTP open-source y documentar config. Prompt `prompts/8. Preparación del sitio para subir vía ftp.txt`
