...
- [ ] **Fase H** — Road to Namecheap: arreglos finales + deploy vía FTP ← **siguiente**
  ...
  - [ ] **H6** — Hero de la landing (NUEVA) ← **siguiente**: reemplazar el video del Hero (render con zoom-out de ~8 min en loop, hoy placeholder) por **imagen(es) HD con efecto**, más livianas y fáciles de cambiar, con posibilidad de **varias al azar**. Probar primero con una imagen tipo Unsplash. Prompt a actualizar/mejorar: `prompts/6. Cambio de hero en Landing.txt`
  - [ ] **H7** — Optimización SEO (sitemap.xml, metadata, OG, preparar Google Search Console; plugin `claude-seo`). GEO/AEO. Prepara archivos para la indexación rápida de la futura Fase I. Prompt `prompts/7. Optimización SEO.txt`
