...
### Flujo para sumar proyectos nuevos a `/proyectos`

> Establecido en la **reapertura de H5** (2026-06-28). Cada vez que se sumen proyectos, el usuario deja **todos los archivos del proyecto** en una subcarpeta de **`web2/proyectos/<Nombre del proyecto>/`** (no se commitea: es material de entrada). Convención por proyecto:
> - `Foto portada/Portada.webp` → portada (card de mini-landing + "espécimen" del Hero de detalle).
> - `Fotos para slider/` (opcional) → galería del detalle. **Si no hay, el proyecto va sin galería** (como el scouting de bombas de calor).
> - PDFs/informes → **fuente de verdad del copy** de ese proyecto (leer y entender antes de redactar; combinar con `que-es-enma.txt`). Para extraer texto: `pdftotext -enc UTF-8 -layout` (hay `pymupdf` disponible).
>
> **Pasos:** (1) copiar imágenes a `public/proyectos/` con nombre versionado (`<slug>-v1.webp`, galería en `public/proyectos/galeria/<slug>-galeria-N-v1.webp`) — ver [[flujo-cambio-imagenes]]; (2) añadir la entrada en **`lib/proyectos.ts`** (mismos campos que la turbina: `lead`, ~7 `facts`, `context`, `did`, `approach`×6, `metrics`×3, `gallery` solo si hay fotos) — copy en 1ª persona, datos reales; (3) la mini-landing, el detalle, `sitemap.ts` y las OG se actualizan solos desde `PROYECTOS`. **El orden de las cards se controla en `CARD_ORDER` dentro de `Proyectos.tsx`** (desktop vs móvil). (4) `typecheck`+`build` + Golden Paths (200 / slug inválido 404).
> - **Confidencialidad:** confirmar con el usuario antes de nombrar clientes/marcas. Ya fijado: el proyecto de upcycling **NO nombra "Cisneverde"** (solo "proyecto de upcycling"); **Colbún SÍ** se nombra en el scouting de bombas de calor.
...
- [x] **Fase F** — Blog ✅ COMPLETA y aprobada por el usuario (2026-06-22). Golden Paths verificados (rutas 200, slug inválido 404). Dos minifases cerradas:
...
- [x] **Página 404** (fuera de numeración H, 2026-06-27, commit `2cb588e`, pusheada a `master`): `app/not-found.tsx` (App Router — captura rutas inválidas y los `notFound()` de `/blog/[slug]` y `/proyectos/[slug]`) + `components/sections/NotFound.tsx` (hero clon de `/vinculacion`: clip-wipe del título + fade + `SignalMarquee`, sección a `min-h-[100svh]` para que el cálido llegue al pie sin Footer). Copy 404 con guiño de marca (imagen **Rey Enma / King Yemma** en `public/404/rey-enma-v1.webp`, `object-contain` sin card) + enlace "Volver al inicio". `robots: noindex`. **OJO H8:** en el export estático esto sale como **`404.html`** → configurarlo como `ErrorDocument` en cPanel.
- [ ] **Fase H** — Road to Namecheap: arreglos finales + deploy ← **siguiente**
  ...
  - [x] **H2** — Rediseño acordeón Vinculación (click en vez de hover, imágenes reales de participaciones). Follow-up 2026-06-26 (commit `efba417`): foto definitiva del Congreso Jóvenes (`congreso-jovenes-futuro-v3.webp`) + fix de carga premium — la foto lazy del acordeón ya no aparece de golpe en móvil (arranca `opacity:0`/`scale(1.02)` y entra con fade+zoom en `onLoad`, respeta `prefers-reduced-motion`).
  ...
  - [x] **H5** — Cambios página Proyectos ✅ COMPLETA y aprobada (2026-06-26). Solo queda «Turbina eólica de baja escala» (CIEP y biodiésel eliminados de `lib/proyectos.ts`, sin ruta de detalle); card única **16:9 a todo el ancho** en `/proyectos` (móvil `aspect-[4/5]`, `sm+` 16:9 estricto; recorte centrado de la foto vertical, encuadre afinable con `object-position`), con la interacción firma foto→ficha (blur + datos + CTA) y **sin pills de dominio** (card y Hero de detalle). Detalle: «Más proyectos» se oculta con un solo proyecto (guard `hasMore`, evita el self-link circular) y el riel final levanta el cálido a `#f3ddbc` para empalmar con el Footer sin costura. Typecheck/build OK; Golden Paths 200/404 OK. `lib/proyectos.ts` + `Proyectos.tsx` + `ProyectoDetalle.tsx`. Prompt `prompts/5. Cambio en la página Proyectos.txt`
    - **Reapertura H5 (2026-06-28) — +2 proyectos.** Se sumaron **«Upcycling de residuos salmoneros»** (`upcycling-residuos-salmoneros`, dominio Economía circular, financiado por Corfo, **con galería** de 5 fotos) y **«Scouting de bombas de calor industriales»** (`scouting-bombas-de-calor`, dominio Energía térmica, reporte para **Colbún** vía OpenBeauchef, **sin galería**). Copy en 1ª persona desde los informes reales en `web2/proyectos/` + `que-es-enma.txt`; **Upcycling no nombra la marca Cisneverde** (se habla solo de "proyecto de upcycling"); **Colbún sí se nombra** (confirmado por el usuario). Cards de `/proyectos` vuelven a **9:16** en grilla `md:grid-cols-3` (revierte la card única 16:9 de H5), con **orden distinto desktop/móvil** (desktop: Upcycling·Turbina·Bombas vía `CARD_ORDER`/`md:order-*`; móvil: Turbina·Upcycling·Bombas) — siguen **sin pills**. Con 3 proyectos reaparece "Más proyectos"/prev-next (`hasMore` true). `sitemap.ts` y OG dinámicos se actualizan solos. Typecheck/build OK (25 páginas); Golden Paths 200/404 OK.
  - [x] **H6** — Hero de la landing ✅ COMPLETA y aprobada (2026-06-26, commits `64c2814` imagen + `291a9cc` código, pusheada a `enma-v2`/`master`). Video placeholder (zoom-out ~8s en loop) → `HeroBackdrop` con `next/image` (`fill`/`priority`/`object-cover`) + **zoom CSS puro 1.2→1.0 en 14s** (parte acercado, termina en la imagen completa, sin paneo, respeta `prefers-reduced-motion`). Imagen de prueba **Cerro Castillo, Aysén** (Unsplash, local en `public/hero/hero-aysen-v1.webp`). **Crédito premium** ("Foto de Carter Obasohan, Cerro Castillo, Aysén.") enlazado al perfil Unsplash con subrayado animado, sustituye al indicador de scroll (se eliminó el keyframe `hero-scroll`). Arquitectura lista para **varias al azar** (array `HERO_IMAGES` + random en `useEffect`, hoy 1 imagen — objetivo "varias" **dejado abierto** a pedido del usuario). Golden Paths OK. Typecheck/build OK. `components/hero/HeroBackdrop.tsx` + `components/sections/Hero.tsx` + `globals.css`. Spec/plan en `docs/superpowers/`. **OJO H8:** el export estático necesitará `images: { unoptimized: true }` en `next.config.ts` para que `next/image` salga en el build FTP.
  - [x] **H7** — Optimización SEO ✅ COMPLETA y aprobada (2026-06-26). **Auditoría `claude-seo` cerrada:** el plugin se instaló y se usó su *expertise* (checklists de `seo-technical`/`seo-schema`/`seo-sitemap`/`seo-geo`/`seo-images`) como **rúbrica aplicada a nuestro código** — su modo nativo (crawl en vivo + Playwright + APIs DataForSEO/Moz/GSC/GA4) **no aplica** a un sitio pre-deploy y choca con las reglas del proyecto. **Veredicto: nada que rehacer**; solo 2 mejoras chicas aplicadas de cara al deploy: **`public/llms.txt`** (guía GEO/AEO para motores de IA, con rutas/datos reales) + **`<html lang>` de `es` → `es-CL`** (alinea con OG `es_CL` / JSON-LD `es-CL`). **Centralización SEO** en `lib/seo.ts` (constantes de marca, `pageMetadata()`, builders JSON-LD) + WhatsApp DRY (número corregido a **+56 9 9337 7935** + mensaje template `?text=` en Hero y Footer). **robots.ts** (allow all + host + sitemap) y **sitemap.ts** dinámico (9 URLs: 5 estáticas + 3 artículos + 1 proyecto). **Metadata** OG/twitter/canonical/`verification.google` (placeholder `REEMPLAZAR_EN_FASE_I`) heredable en `layout.tsx` + por página (Home, internas, `[slug]`). **OG images branded por código** (`lib/og.tsx` con `next/og` ImageResponse, Manrope vía Fontsource con fallback a fuente default; default del sitio + dinámicas por artículo/proyecto) — compila bajo build, **no se necesitó el Plan B estático**. **JSON-LD** `Organization`+`WebSite` (sitio), `Article` (blog), `BreadcrumbList` (internas + detalle) vía `components/seo/JsonLd.tsx`. **Docs**: `docs/seo/google-search-console.md` (guía Fase I, meta tag + archivo HTML) y `docs/seo/geo-aeo-notas.md`. Fix colateral: description de `/proyectos` corregida post-H5 (solo turbina). Typecheck/build OK (21 rutas prerenderizadas), Golden Paths 200/404 OK. Spec/plan en `docs/superpowers/`. **OJO H8:** `images:{ unoptimized:true }` en `next.config.ts` para que `next/image` y las OG salgan en el export FTP. Prompt `prompts/7. Optimización SEO.txt`
  - [x] **H7.5 — Auditoría GEO** ✅ COMPLETA (2026-06-28). Correcciones GEO a partir de una *AI Visibility Audit* hecha con **Goodie AI** (higoodie.com, score inicial **61%** sobre el preview Vercel). Aplicado, centralizado en `lib/seo.ts` siguiendo el método SEO del proyecto: **(1)** fix del **bug real de OG** — las páginas índice internas (Nosotros/Proyectos/Vinculación/Blog) NO tenían `og:image` (Next no cascada la OG raíz a segmentos hijos) → `pageMetadata()` ahora inyecta la OG branded por defecto, con `ogImage:null` en los detalles blog/proyecto para conservar su OG dinámica por slug; **(2)** **`Article` JSON-LD** en los 3 detalles de proyecto (headline+`datePublished`+`dateModified`, `proyectoJsonLd()`); **(3)** **`FAQPage` JSON-LD** en la home (`lib/faq.ts` = espejo en texto plano de `FAQ.tsx`, mantener en sync); **(4)** **byline visible** "Por Equipo Enma · Actualizado \<fecha\>" en el detalle de proyecto (campo `published` real, no inventado) → señal de frescura; **(5)** **RSS** `/feed.xml` (route `force-static`) + `<link rel=alternate>` en `<head>`; **(6)** **`public/llms-full.txt`** (artículos y proyectos a texto completo) referenciado desde `llms.txt`; **(7)** `robots.ts` dinámico → **`public/robots.txt` estático** con `Content-Signal` + allow explícito a bots de IA (GPTBot/ClaudeBot/PerplexityBot/Google-Extended/etc.) — la API de robots de Next no emite directivas custom; **(8)** meta `robots`/`googlebot` con `max-snippet:-1, max-image-preview:large`; **(9)** **Link headers** (sitemap+feed) en `.htaccess`; **(10)** **alt descriptivo** en las 6 fotos de Servicios. **Omitido a propósito** (no aplica a un sitio corporativo estático, sería deshonesto): todo lo de **Agent Discovery (MCP server card, A2A, agent-skills, API catalog, OpenAPI, WebMCP, Web Bot Auth)** y **Authentication & API (OAuth/OIDC/status API/PKCE)** — Enma no expone API ni agente. **Dependiente del hosting** (no del código): HTTP/2/3 (Apache Namecheap); los Link headers requieren `mod_headers`. El **score real solo sube tras re-desplegar (H8) y re-escanear** — el audit corrió sobre el preview Vercel. Typecheck/build OK (25 páginas), Golden Paths 200/404 OK.
  - [x] **H7.6 — Auditoría Lighthouse (page-by-page)** ✅ COMPLETA (2026-06-29, commit final `066e55d`). Reapertura de la mini-fase H de SEO a partir de auditorías **Lighthouse / PageSpeed Insights** (extensión Chrome) sobre el preview Vercel, **móvil + desktop**, corrigiendo **una página por sesión** (commit+push y `/clear` entre páginas). **Regla acordada con el usuario:** se aplican TODOS los hallazgos de Lighthouse **excepto el de contraste de color** ("Los colores de fondo y de primer plano no tienen una relación de contraste adecuada") — el resto de Accesibilidad SÍ se aplica. Los reportes los entrega el usuario como PDF (no usar Playwright; la API anónima de PageSpeed da 429). **REGLA DE ORO descubierta:** los PDF llegan **desfasados** del preview pre-deploy — verificar SIEMPRE el tamaño real en disco con PIL antes de re-encodear. **Pendiente global:** re-escanear post-deploy (los scores reales suben en Apache/H8: los `.htaccess` no aplican en Vercel).
    - [x] **Landing (`/`)** ✅ COMPLETA y pusheada (commit `946cc02`, 2026-06-28). Móvil 69 / desktop 83 → correcciones: **(1) imágenes** — re-encode (Pillow, q72-78) de las 10 fotos del reporte a dimensiones razonables, nombres versionados nuevos (`-v2`, servicio-04 `-v4`, `logo-verde-v2`) y borrado de las viejas; **métrica 1976 KB → 268 KB**, total landing **4.75 MB → 1.16 MB (-3.6 MB)**; ataca "cargas útiles de red", "entrega de imágenes" y LCP. **(2)** El `<dl>` de **About** anidaba dos `<div>` antes de `<dt>`/`<dd>` → aplanado a un `<div>` por par (regla axe `dlitem`): resuelve las **2 alertas de accesibilidad** y la de **Navegación con agentes** (2/3 → árbol de accesibilidad malformado por lo mismo). **(3)** `browserslist` moderno en `package.json` → Next deja de inyectar **polyfills heredados** (Array.prototype.at/flat/flatMap, Object.fromEntries/hasOwn…). **(4)** Headers de seguridad seguros en `public/.htaccess` (X-Frame-Options, COOP, nosniff, Referrer-Policy) — **omitidos a propósito CSP anti-XSS y Trusted Types** (romperían los scripts/estilos inline del export estático de Next). Typecheck/build OK (25 páginas), Golden Paths OK. **OJO:** los headers `.htaccess` solo aplican en el hosting Apache (H8), no en el preview Vercel; el score real sube al re-escanear tras push/deploy.
    - [x] **Páginas internas (lote móvil)** ✅ (commit `28cd1fb`, 2026-06-28). 6 PDF móviles (blog índice, proyectos, vinculación, turbina, scouting-bombas, upcycling). Foco imágenes + Navegación con agentes; ignorar animaciones/estética y contraste. Fixes: portadas blog patagonia **777→204** / aysén **525→135** KiB (1400px), hero índice blog **235→72** KiB (760px), galería turbina re-encode; **NavBar `inert={!menuOpen}`** en el drawer (saca los `<Link>` enfocables del árbol cuando cerrado → corrige `aria-hidden-focus` = Accesibilidad + Navegación con agentes en TODAS las páginas). Detalle Upcycling desktop ✅ aparte (commit `a50bc3f`: galería 1600px). Los PDF venían desfasados (cards/galería ya arregladas en disco).
    - [x] **Artículos del blog (móvil)** ✅ (commit `066e55d`, 2026-06-29). 3 PDF (enma-explicado-en-facil, patagonia-se-resuelve, energia-mas-cara-aysen). De nuevo desfasados: patagonia/aysén ya en `-v2` y el `inert` ya estaba (confirmado en el export: `aria-hidden="true" inert=""` con drawer cerrado). Único fix real = **portada-enma** seguía en `-v1` → re-encode a 1400px q74 (**84→30 KiB**), bump `-v2`, ref en `lib/blog.ts`, v1 borrada. Typecheck/build OK (25 páginas), Golden Paths 200/404 OK.
  - [ ] **H8** — **Primera subida a producción, MANUAL vía cPanel File Manager (.zip)** ← **siguiente** (EN PAUSA: esperando aprobación de los cofundadores de Enma antes de generar el `.zip`). Decisión del usuario (2026-06-27): la **primera** subida la hace él subiendo un `.zip` y descomprimiéndolo desde el File Manager por defecto de cPanel (ya sabe hacerlo). Tarea de Claude: (1) activar export estático — `output: "export"` + `images:{ unoptimized:true }` en `next.config.ts` (lo necesitan `next/image` del Hero y las OG de `next/og`); (2) build y **seleccionar los archivos de producción**; (3) verificar que en `out/` salgan todas las rutas como HTML (incl. **`404.html`**), `_next/`, `robots.txt`, `sitemap.xml`, `llms.txt`, OG (`opengraph-image`) y todo `public/`; (4) **comprimir Claude mismo el `out/` en un `.zip`** listo para subir a `public_html` (si no se puede, dejar la carpeta lista + instrucciones de zipeo); (5) **guiar paso a paso** al usuario: subir zip → descomprimir en `public_html` → configurar dominio enmachile.com (document root) + `404.html` como `ErrorDocument`; (6) verificar Golden Paths + SEO en vivo (sin Playwright). **NADA de FTP en esta fase.** Prompt en el chat (el usuario lo pega).
  - [ ] **H9 (ÚLTIMA FASE)** — **Migración a flujo FTP** para que actualizar el sitio sea tan simple como GitHub + Vercel: elegir cliente/herramienta FTP open-source, documentar config y dejar el flujo de actualización listo. Solo DESPUÉS de que H8 esté en producción y verificado. Tras confirmar el sitio en vivo, abrir **Fase I** (indexación rápida) replicando la estructura de prompts de la Fase H.
